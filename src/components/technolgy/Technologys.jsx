...
const Technolgys = () => {
    return (
        <div className='grid grid-cols-3 container mx-auto justify-center items-center gap-10 py-10'>
            {
                data.map(item => <>
                    <Link href={`/technologies/${item.id}?search=${JSON.stringify(item)}`} key={item.id}>
                        <div className='hover:shadow-2xl h-full hover:rounded-xl group overflow-hidden'>
                            <div className=''>
                                <Image height={1000} width={1000} src={item.image} alt={item.name} className='p-2 h-96 group-hover:scale-110 duration-500 transform ease-in-out' />
                            </div>
                            <div className='p-5'>
